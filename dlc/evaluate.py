...
def analyze_video(config_path, vid,shuffle = 1,  make_video = True, filterpredictions = True, filtertype = 'median', destfolder = None):
    if destfolder == None:
        destfolder = os.path.dirname(vid)
    scorername = deeplabcut.analyze_videos(config_path, vid, videotype='.mp4',auto_track = True, robust_nframes = True, save_as_csv = True, shuffle = shuffle, destfolder = destfolder)
    ...
    if make_video:
        ...
        try:
            ...
            print(f'Deeplabcut fails to detect the object!')
