...
camera_matrix_1 = np.array([[1400.59, 1030.9, 0 ], [0, 1400.59, 526.489], [0, 0, 1]])
camera_matrix_2 = np.array([[1399.43, 1059.85, 0], [0, 1399.43, 545.848], [0, 0, 1]])
    ...
    for i in range(len(video_list)):
        ...
        for j in range(len(video_list[i])):  # looping over cameras
            if cam_names[j] in video_list[i][j]:
                ...
                if track_method == 'box':
                    tr_method_suffix = '_bx'
                elif track_method == 'ellipse':
                    tr_method_suffix = '_el'
        ...
        if run_triangulate:
            ...
            h5_left = [h5name for h5name in dataname if 'left' in h5name][0]
            h5_right = [h5name for h5name in dataname if 'right' in h5name][0]
