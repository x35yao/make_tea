...
def create_empty_df(dataframe, scorer, flag):
    ...
    scorer_input = df.columns.get_level_values("scorer").unique()[0]
    a = np.full((df.shape[0], 3), np.nan)
    dataFrame = None
    # print(dataframe)
    for individual in individuals:
        bodyparts = df[scorer_input][individual].columns.get_level_values("bodyparts").unique()
        for bodypart in bodyparts:
            if flag == "2d":
                ...
                pdindex = pd.MultiIndex.from_product(
                    [[scorer], [individual], [bodypart], ["x", "y", "z"]],
                    names=["scorer", "individuals","bodyparts", "coords"],
                )
