...
if __name__ == '__main__':
    ...
    demos_total = sorted(list(gripper_trajs_in_ndi.keys()))

    middle_pool = [d for d in demos_total[:10] if d in train_demos_pool]
    left_pool = [d for d in demos_total[10:20] if d in train_demos_pool]
    right_pool = [d for d in demos_total[30:40] if d in train_demos_pool]
    d1_pos, d2_pos, d3_pos, d4_pos = [], [], [], []
    d1_ori, d2_ori, d3_ori, d4_ori = [], [], [], []
    for i in trange(n_tests):
        train_left = random.sample(left_pool, k=5)
        train_middle = random.sample(middle_pool, k=1)
        train_right = random.sample(right_pool, k=1)
        ...
        test_demos = random.sample(right_pool, k=1)
        ...
        # start[:3] = H_puck_in_ndi[:-1, 3] + np.array([0, -10, 0])
