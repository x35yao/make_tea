...
if __name__ == '__main__':
    ...
    for i in trange(n_tests):
        ...
        for individual in sorted(individuals):
            ...
            test_data_frame, _ = prepare_data_for_pmp(gripper_traj_in_generalized_obj, individual, test_demos, data.dims)
            test_data_all_frames.append(np.array(test_data_frame))
        Tn = len(times[0])
        ...
        ind = model_pmp.model.select_mode(t_pmp, test_data_all_frames[0])
        mu_mean_tp_pmp1, sigma_mean_tp_pmp1 = predict2(tp_pmps, t_pmp, test_demo, HTs_generalized_obj_in_ndi, individuals, mode_selected=ind)
        ...
        if PLOT:
            ...
            for i, ax in enumerate(axes):
                ax.plot(mu_pos_tp_pmp1[:, i + 3], '-', color='blue', label='1')
                # ax.plot(mu_pos_tp_pmp2[:, i + 3], '-', color='yellow', label='2')
